...
// Specialized function for disease detection analysis
export const analyzeImageWithText = async (imageData: string, prompt: string): Promise<string> => {
  try {
    if (!genAI) {
      throw new Error("Gemini AI is not properly initialized. Please check your API key configuration.");
    }
    ...
    console.error("Error in disease detection analysis:", error);
    ...
    if (error.message?.includes("API_KEY_INVALID")) {
      ...
      throw new Error("Content was blocked by safety filters. Please try with a different image.");
    } else {
